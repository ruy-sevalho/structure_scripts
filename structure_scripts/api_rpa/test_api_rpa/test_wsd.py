...
circle_section_1 = CircularSectionGeometry(
    outer_diameter=50 * mm, wall_thickness=2 * mm
)
section_1 = CircularSection(geometry=circle_section_1, material=steel)
compression_1 = AxialCompression(profile=section_1, length=1000 * mm)
...
def test_radius_gyration(profile: tuple[CircularSectionGeometry, float]):
    calculated, reference = same_units_simplify(
        profile[0].radius_of_gyration, profile[1]
    )
...
def test_compressive_stress_a(profile: tuple[AxialCompression, Quantity]):
    calculated, reference = same_units_simplify(
        profile[0].allowable_stress_a, profile[1]
    )
...
def test_compressive_stress_b(profile: tuple[AxialCompression, Quantity]):
    calculated, reference = same_units_simplify(
        profile[0].allowable_stress_b, profile[1]
    )
    assert calculated == approx(reference)
...
def test_compressive_stress(profile: tuple[AxialCompression, Quantity]):
    calculated, reference = same_units_simplify(
        profile[0].allowable_stress, profile[1]
    )
    assert calculated == approx(reference)
...
def test_acting_compressive_stress(
    beam: tuple[AxialCompression, Quantity, Quantity]
):
    """First quantity is load applied and second expected stress"""
    calculated, reference = same_units_simplify(
        beam[0].acting_stress(load=beam[1]), beam[2]
    )
    assert calculated == approx(reference)
