...
def _c_coefficient_collum_buck(yield_stress: Quantity, modulus_linear: Quantity):
    return (ratio_simplify(modulus_linear, yield_stress) * 2 * math.pi ** 2) ** 0.5


def _allowable_compressive_stress_a(slenderness: float, c: float, yield_stress: Quantity):
    return (
            (1 - slenderness ** 2 / (2 * c ** 2)) * yield_stress /
            (
                    5. / 3. + 3 * slenderness / (8 * c) - slenderness ** 3 / (8 * c ** 3)
            )
    )


def _allowable_compressive_stress_b(slenderness: float, modulus_linear: Quantity):
    return (
            12 * math.pi ** 2 * modulus_linear / (23 * slenderness ** 2)
    )
...
def max_torsional_shear_stress(torsional_moment: Quantity, polar_inertia: Quantity, outer_diameter: Quantity):
...
def combined_bending_compression_criteria_a(
    ...
    """Eq 6.20"""
    first_term = ratio_simplify(acting_compressive_stress, allowable_compressive_stress)
    second_term_numerator = modification_coefficient * (
            acting_bending_stress_x ** 2 + acting_bending_stress_y ** 2
    ) ** 0.5
