...
alpha = Alpha()
lambda_ = Lambda()
axis_index_table = {"major": "x", "minor": "y"}
...
def _wrapper(start: str, content: str, end: str):
    return env.get_template("wrapper.tex").render(
...
def multline(content: str):
    return _wrapper(
        start=r"\begin{multline*}", content=content, end=r"\end{multline*}"
    )
...
def _get_header(
    ...
    if (
        unit_display == "header" and not units == percent
    ):  # or units == criteria:
...
def _process_quantity_entry_to_string(
    ...
    return LatexQuantity(
        entry, options={"round-precision": round_precision}
    ).dumps()
...
def _dataframe_table_columns(
    ...
    for row, name in enumerate(df.columns):
        ...
        if isinstance(instance, Quantity):
            ...
            header = _get_header(
                label=label, units=units, unit_display=unit_display
            )
        else:
            header = label
...
def _add_df_table(
    ...
    tblr = Center()
    tblr.append(
        _dataframe_table_rows(
            df=df, config_dict=config_dict, unit_display="header"
        )
    )
    table.append(tblr)
    return table
...
def _member_slenderness_minor_axis_flexural_bucking_latex(
    ...
    template = env.get_template(
        "member_minor_axis_flexural_buckling_slenderness.tex"
    )
    return wrapper(
        template.render(
            factor_k=factor_k,
...
def _axial_compression_non_slender_critical_stress_lower_than(
    ...
    template = env.get_template(
        "axial_compression_critical_stress_lower_than.tex"
    )
    return wrapper(
        template.render(
            yield_stress=yield_stress,
...
def _axial_compression_non_slender_critical_stress_greater_than(
    ...
    template = env.get_template(
        "axial_compression_critical_stress_greater_than.tex"
    )
    return wrapper(
        template.render(
            yield_stress=yield_stress,
...
def _design_strength(
    ...
    safety_factor: str,
    design_strength: str,
    safety_factor_type: DesignType,
    strength_type: Literal["force", "moment"],
    wrapper: Callable[[str], str] = standard_wrapper,
):
    table = {
        DesignType.ASD: _design_strength_asd,
        DesignType.LRFD: _design_strength_lfrd,
    }
    return table[safety_factor_type](
        nominal_strength=nominal_strength,
        safety_factor=safety_factor,
        design_strength=design_strength,
...
def _flexural_lateral_torsional_buckling_strength_case_b(
    ...
    template = env.get_template(
        "flexural_lateral_torsional_buckling_strength_case_b.tex"
    )
    return wrapper(
        template.render(
...
def _build_doc(document: str, date: str = datetime.now()):
    ...
    return template.render(body=document, date=date)
...
def _flexure_compression_h1_criteria_equation(
    ...
    force_factor = {"lower_than_0_2": r"2 \cdot", "greater_than_0_2": ""}
    moment_factor = {"lower_than_0_2": "", "greater_than_0_2": r"\frac{8}{9}"}
    return wrapper(
...
TABLE_ELEMENT = {"flange": ("flange", "f"), "web": ("alma", "w")}
...
def _flexural_slenderness_result(slenderness: Slenderness, element: str):
    table_slenderness = {
...
def save_single_entry(content: str, file_name: str):
    path = (
        pathlib.Path(
            r"C:\Users\U3ZO\OneDrive - PETROBRAS\Documentos\pdf_maker\tex_files"
        )
        / file_name
    )
