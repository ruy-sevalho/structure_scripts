...
from structure_scripts.shared.latex_helpers import (
    ...
    _elastic_buckling_critical_stress_latex,
    _axial_compression_non_slender_critical_stress_lower_than,
    _axial_compression_non_slender_critical_stress_greater_than,
    _axial_compression_nominal_strength,
    _process_quantity_entry_config,
    _design_strength,
    _flexural_yield_nominal_strength,
    _limit_length_yield,
    _limit_length_lateral_torsional_buckling,
    _flexural_lateral_torsional_buckling_strength_case_b,
    _build_doc,
    CONCATENATE_STRING,
    _effective_radius_of_gyration_equation,
    _flexure_compression_h1_criteria_equation,
    _axial_strength_ratio_equation,
    _ratio_equation,
    standard_wrapper,
    Multline,
    env,
    _axial_slenderness_result,
...
SLENDER_NON_SLENDER_LIMIT_DESCRIPTION = NoEscape(
    r"Raz\~ao de esbletez limite n\~ao esbelto / esbelto:"
)
COMPACT_NON_COMPACT_LIMIT_DESCRIPTION = NoEscape(
    r"Raz\~ao de esbletez limite compacto / n\~ao compacto:"
)
NON_COMPACT_SLENDER_LIMIT_DESCRIPTION = NoEscape(
    r"Raz\~ao de esbletez limite n\~ao compacto / esbelto:"
)
...
class AreaPropertiesLatex:
    ...
    def area(self):
        return _process_quantity_entry_config(
            entry=self.data.area, print_config=config_dict.area
        )

    @cached_property
    def major_axis_inertia(self):
        return _process_quantity_entry_config(
            entry=self.data._major_axis_inertia,
            print_config=config_dict._major_axis_inertia,
        )

    @cached_property
    def major_axis_elastic_section_modulus(self):
        return _process_quantity_entry_config(
            entry=self.data._major_axis_elastic_section_modulus,
            print_config=config_dict._major_axis_elastic_section_modulus,
        )

    @cached_property
    def major_axis_plastic_section_modulus(self):
        return _process_quantity_entry_config(
            entry=self.data._major_axis_plastic_section_modulus,
            print_config=config_dict._major_axis_plastic_section_modulus,
        )

    @cached_property
    def major_axis_radius_of_gyration(self):
        return _process_quantity_entry_config(
            entry=self.data._major_axis_radius_of_gyration,
            print_config=config_dict._major_axis_radius_of_gyration,
        )

    @cached_property
    def minor_axis_inertia(self):
        return _process_quantity_entry_config(
            entry=self.data._minor_axis_inertia,
            print_config=config_dict._minor_axis_inertia,
        )

    @cached_property
    def minor_axis_elastic_section_modulus(self):
        return _process_quantity_entry_config(
            entry=self.data._minor_axis_elastic_section_modulus,
            print_config=config_dict._minor_axis_elastic_section_modulus,
        )

    @cached_property
    def minor_axis_plastic_section_modulus(self):
        return _process_quantity_entry_config(
            entry=self.data._minor_axis_plastic_section_modulus,
            print_config=config_dict._minor_axis_plastic_section_modulus,
        )

    @cached_property
    def minor_axis_radius_of_gyration(self):
        return _process_quantity_entry_config(
            entry=self.data._minor_axis_radius_of_gyration,
            print_config=config_dict._minor_axis_radius_of_gyration,
        )

    @cached_property
    def torsional_constant(self):
        return _process_quantity_entry_config(
            entry=self.data._torsional_constant,
            print_config=config_dict._torsional_constant,
        )
    ...
    def warping_constant(self):
        return _process_quantity_entry_config(
            entry=self.data.doubly_symmetric_i_warping_constant,
            print_config=config_dict.doubly_symmetric_i_warping_constant,
        )
...
class DoublySymmetricIFlangeAxialCompressionSlendernessLatex:
    ...
    def ratio(self):
        return _process_quantity_entry_config(
            entry=self.model.slenderness_ratio,
    ...
    def ratio_equation(self):
        return _ratio_equation(
            ...
            numerator_value=self.model.section.dimensions.latex.flange_width[
                1:-1
            ],
            denominator_symbol=f"2{config_dict.flange_thickness[1:-1]}",
            denominator_value=self.model.section.dimensions.latex.flange_thickness,
    ...
    def slender_limit_ratio_rolled(self):
        return _slenderness_default_limit_ratio_latex(
            factor="0.56",
            modulus=self.model.section.material.latex.modulus_linear,
            yield_stress=self.model.section.material.latex.yield_stress,
    ...
    def limit_ratio(self):
        ...
        return table[self.model.section.construction]

    @cached_property
    def slenderness_result(self):
        return _axial_slenderness_result(
            slenderness=self.model.axial_compression_value, element="flange"
        )

    def resume(self):
        save_single_entry(
            content=self.limit_ratio, file_name="comp_flange_slender_limit.tex"
        )
    ...
    def resume_latex(self):
        return Subsubsection(
            title="Flange",
            data=[
                SLENDER_NON_SLENDER_LIMIT_DESCRIPTION,
                NoEscape(self.limit_ratio),
...
class DoublySymmetricIFlangeFlexuralMajorAxisSlendernessLatex:
    ...
    def slenderness_result(self):
        return _flexural_slenderness_result(
            slenderness=self.model.flexural_major_axis_value, element="flange"
        )
...
class DoublySymmetricIFlangeFlexuralMinorAxisSlendernessLatex:
    ...
    def slenderness_result(self):
        return _flexural_slenderness_result(
            slenderness=self.model.flexural_minor_axis_value, element="flange"
        )
...
class DoublySymmetricIWebAxialCompressionSlendernessLatex:
    ...
    def axial_compression_limit_ratio(self):
        return _process_quantity_entry_config(
            entry=Quantity(self.model.axial.limit_ratio),
            print_config=config_dict.compact_limit_ratio,
        )
    ...
    def resume(self):
        save_single_entry(
            content=self.limit_ratio, file_name="comp_web_slender_limit.tex"
        )

    @cached_property
    def slenderness_result(self):
        return _axial_slenderness_result(
            slenderness=self.model.axial_compression_value, element="web"
        )
...
class DoublySymmetricIWebFlexuralSlendernessLatex:
    ...
    def flexural_slender_limit_ratio(self):
        return _process_quantity_entry_config(
            entry=Quantity(self.model.flexural_non_compact_slender_limit),
            print_config=config_dict.slender_limit_ratio,
        )
    ...
    def flexural_compact_limit_ratio(self):
        return _process_quantity_entry_config(
            entry=Quantity(self.model.flexural_compact_non_compact_limit),
            print_config=config_dict.compact_limit_ratio,
        )
    ...
    def slenderness_results(self):
        return _flexural_slenderness_result(
            slenderness=self.model.flexural_value, element="web"
        )
...
class DoublySymmetricIAxialCompressionSlendernessLatex:
    ...
    def resume_latex(self):
        return Subsection(
            ...
            data=[self.flange.resume_latex, self.web.resume_latex],
        )
...
class DoublySymmetricISlendernessLatex:
    ...
    def flange_slenderness(self):
        return _process_quantity_entry_config(
            entry=self.model.flange.slenderness_ratio,
            print_config=config_dict.flange_slenderness,
        )
    ...
    def flange_slenderness_equation(self):
        return _ratio_equation(
            ...
            denominator_value="2 "
            + r"\cdot "
            + self.model.section.dimensions.latex.flange_thickness,
            ratio_symbol=config_dict.flange_slenderness.label[1:-1],
    ...
    def web_slenderness(self):
        return _process_quantity_entry_config(
            entry=self.model.web.slenderness_ratio,
    ...
    def resume_ratios(self):
        ...
        save_single_entry(
            content=self.web_slenderness, file_name="web_slenderness_ratio.tex"
        )

    @cached_property
    def resume_latex(self):
        flange = Subsubsection(
            title="Flange", data=NoEscape(self.flange_slenderness_equation)
        )
        web = Subsubsection(
            title="Alma", data=NoEscape(self.web_slenderness_equation)
        )
        return Subsection(
            title=NoEscape(r"Raz\~oes de Esbeltez"), data=[flange, web]
        )
...
class DoublySymmetricIUserDefinedLatex:
    ...
    def resume(self):
        save_single_entry(content=self.data_table, file_name="profile.tex")
        save_single_entry(
    ...
    def limit_length_flexural_yield_equation(self):
        return _limit_length_yield(
            minor_axis_radius_of_gyration=self.model.area_properties.latex._minor_axis_radius_of_gyration,
    ...
    def warping_constant(self):
        return _process_quantity_entry_config(
            entry=self.model.doubly_symmetric_i_warping_constant,
            print_config=config_dict.doubly_symmetric_i_warping_constant,
        )
    ...
    def effective_radius_of_gyration_equation(self):
        return _effective_radius_of_gyration_equation(
            minor_axis_inertia=self.model.area_properties.latex._minor_axis_inertia,
            warping_constant=self.model.latex.doubly_symmetric_i_warping_constant,
            major_axis_section_modulus=self.model.area_properties.latex._major_axis_elastic_section_modulus,
    ...
    def limit_length_flexural_lateral_torsional_buckling_equation(self):
        return _limit_length_lateral_torsional_buckling(
            ...
            torsional_constant=self.model.area_properties.latex._torsional_constant,
            coefficient_c=self.coefficient_c,
            distance_between_centroids=self.model.dimensions.latex.distance_between_centroids,
            elastic_major_axis_section_modulus=self.model.area_properties.latex._major_axis_elastic_section_modulus,
        )

    @cached_property
    def slenderness(self):
        return DoublySymmetricISlendernessLatex(model=self.model.slenderness)
...
class BeamCompressionEffectiveLengthLatex:
    ...
    def required_strength(self):
        return _process_quantity_entry_config(
            entry=self.model.required_strength_major_axis,
    ...
    def minor_axis_slenderness_equation(self):
        ...
        inequality_sign = (
            r"\leq" if slenderness_value <= slenderness_limit else ">"
        )
        return _member_slenderness_minor_axis_flexural_bucking_latex(
            factor_k=self.factor_k_minor_axis,
            length=self.unbraced_length,
            radius_of_gyration=self.model.section.area_properties.latex._minor_axis_radius_of_gyration,
    ...
    def non_slender_critical_stress(self):
        if (
            self.model.minor_axis_slenderness
            <= self.model.member_slenderness_limit
        ):
    ...
    def nominal_strength_equation(self):
        return _axial_compression_nominal_strength(
            critical_stress=self.flexural_buckling_critical_stress,
            area=self.model.section.area_properties.latex.area,
    ...
    @cached_property
    def resume(self):
        self.model.section.latex.slenderness.axial.resume()
        save_single_entry(
    ...
    @cached_property
    def resume_latex(self):
        slenderness = self.model.section.latex.slenderness.axial.resume_latex
        nominal_critical_strength = NoEscape(
            standard_wrapper(r"P_n = F_{cr} A_g")
        )
        nominal_critical_strength_terms = NoEscape(
            env.get_template("carga_critica_compressao_termos.tex").render()
        )
        critical_stress_formulation = NoEscape(
            env.get_template("comp_critical_stress_formulation.tex").render()
        )
...
class BeamFlexureMajorAxisDoublySymmetricLatex:
    ...
    def resume(self):
        self.model.section.latex.slenderness.flexural_major_axis.resume()
        save_single_entry(
            content=self.yield_strength_equation,
            file_name="flex_major_axis_yield_strength.tex",
        )
        save_single_entry(
            content=self.model.section.latex.limit_length_flexural_yield_equation,
            file_name="limit_lenght_flex_yidel.tex",
        )
        save_single_entry(
            content=self.model.section.latex.effective_radius_of_gyration_equation,
            file_name="effective_radius_of_gyration.tex",
        )
        save_single_entry(
            content=Multline(
                data=NoEscape(
                    self.model.section.latex.limit_length_flexural_lateral_torsional_buckling_equation
                )
            ).dumps(),
            file_name="limit_length_flex_lateral_torsional_buck.tex",
        )
        save_single_entry(
            content=self.design_strength, file_name="flex_design_strengths.tex"
        )

    @cached_property
    def resume_latex(self):
        slenderness = (
            self.model.section.latex.slenderness.flexural_major_axis.resume_latex
        )
        ...
        lateral_torsional_formulation = NoEscape(
            env.get_template(
                "flex_lateral_torsional_strength_formulation.tex"
            ).render()
        )
        limit_length_lateral_torsional_buckling_formulation = NoEscape(
            env.get_template(
                "limit_length_lateral_torsional_buckling_formulation.tex"
            ).render()
        )
        lateral_torsional_buck = Subsubsection(
            ...
            data=[
                ...
                NoEscape(
                    self.model.section.latex.limit_length_flexural_yield_equation
                ),
                limit_length_lateral_torsional_buckling_formulation,
                NoEscape(
                    self.model.section.latex.effective_radius_of_gyration_equation
                ),
                NoEscape(
                    self.model.section.latex.limit_length_flexural_lateral_torsional_buckling_equation
                ),
                NoEscape(
                    self.model.latex.major_axis.lateral_torsional_buckling_strength_equation
                ),
            ],
        )
        calculation = Subsection(
    ...
    def yield_strength_equation(self):
        return _flexural_yield_nominal_strength(
            yield_stress=self.model.section.material.latex.yield_stress,
            plastic_section_modulus=self.model.section.area_properties.latex._major_axis_plastic_section_modulus,
    ...
    def lateral_torsional_buckling_strength_case_b_equation(self):
        return _flexural_lateral_torsional_buckling_strength_case_b(
            ...
            elastic_major_axis_section_modulus=self.model.section.area_properties.latex._major_axis_elastic_section_modulus,
            unbraced_length=self.unbraced_length,
            limit_length_yield=self.model.section.latex.limit_length_flexural_yield,
            limit_length_lateral_torsional_buckling=self.model.section.latex.limit_length_flexural_lateral_torsional_buckling,
    ...
    def lateral_torsional_buckling_strength_equation(self) -> str | None:
        ...
        return flexural_lateral_torsional_buckling_strength(
            ...
            limiting_length_yield=self.model.section.limit_length_yield,
            limiting_length_torsional_buckling=self.model.section.limit_length_torsional_buckling,
        )
...
class BeamFlexureMinorAxisDoublySymmetricLatex:
    ...
    def resume(self):
        self.model.section.latex.slenderness.flexural_minor_axis.resume()
        save_single_entry(
            content=self.yield_strength_equation,
    ...
    def resume_latex(self):
        slenderness = (
            self.model.section.latex.slenderness.flexural_minor_axis.resume_latex
        )
        calculation = Subsection(
            title=NoEscape(r"Carga cr\'itica"),
            data=insert_between(
        ...
        return Section(
            ...
            data=insert_between([slenderness, calculation]),
        )
    ...
    def yield_strength_equation(self):
        return _flexural_yield_nominal_strength(
            yield_stress=self.model.section.material.latex.yield_stress,
            plastic_section_modulus=self.model.section.area_properties.latex._minor_axis_plastic_section_modulus,
            nominal_strength=self.yield_strength,
            axis="minor",
        )
...
class BeamCompressionFlexureDoublySymmetricEffectiveLengthLatex:
    ...
    def resume(self):
        self.model.section.material.latex.resume()
    ...
    @cached_property
    def resume_latex(self):
        material = self.model.section.material.latex.resume_latex
    ...
    def critical_loads_report(self):
        ...
        material = self.model.section.material.latex.data_table
        title_material = r"\section{Material}"
        profile_properties = self.model.section.latex.data_table
        slenderness_tile = r"\subsection{Slenderness ratio}"
        flange_slenderness_title = r"\subsubsection{Flange}"
        flange_slenderness = (
            self.model.section.latex.slenderness.flange_slenderness_equation
        )
        web_slenderness_title = r"\subsubsection{Web}"
        web_slendernses = (
            self.model.section.latex.slenderness.web_slenderness_equation
        )
